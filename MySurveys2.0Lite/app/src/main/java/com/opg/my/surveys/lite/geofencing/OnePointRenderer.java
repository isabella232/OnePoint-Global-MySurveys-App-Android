...
import com.opg.my.surveys.lite.Fragments.SurveyByLocationFragment;
...
/**
 * Created by kiran on 05-01-2018.
 */

/**
 * The default view for a ClusterManager. Markers are animated in and out of clusters.
 */
public class OnePointRenderer<T extends ClusterItem> implements ClusterRenderer<T> {
    ...
    private LinkedHashMap<LatLng,Circle> hashMap = new LinkedHashMap();
   // private Spa
    ...
    public OnePointRenderer(Context context, GoogleMap map, ClusterManager<T> clusterManager) {
        mContext = context;
        mMap = map;
        mAnimate = true;
    ...
    private class RenderTask implements Runnable {
        ...
        @SuppressLint("NewApi")
        public void run() {
            if (clusters.equals(OnePointRenderer.this.mClusters)) {
                mCallback.run();
                return;
            }
            ...
            if (OnePointRenderer.this.mClusters != null && SHOULD_ANIMATE) {
                existingClustersOnScreen = new ArrayList<Point>();
                for (Cluster<T> c : OnePointRenderer.this.mClusters) {
            ...
            for (final MarkerWithPosition marker : markersToRemove) {
                ...
                ((Activity)mContext).runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        for(Circle circle : hashMap.values())
                        {
                            if(marker.position.longitude == circle.getCenter().longitude && marker.position.latitude == circle.getCenter().latitude )
                            {
            ...
            mMarkers = newMarkers;
            OnePointRenderer.this.mClusters = clusters;
            mZoom = zoom;
    ...
    private void drawRadius(ClusterItem point) {
        SurveyByLocationFragment.MyItem myItem = (SurveyByLocationFragment.MyItem) point;
        CircleOptions circleOptions = new CircleOptions()
                .center(point.getPosition())
                .fillColor(Color.alpha(Color.parseColor(MySurveysPreference.getThemeActionBtnColor(mContext))))
                .strokeColor(Color.parseColor(MySurveysPreference.getThemeActionBtnColor(mContext)))
                .radius(myItem.getRadius())
                .strokeWidth(2);
        Circle circle =   mMap.addCircle(circleOptions);
        //circleList.add(circle);
        hashMap.put(point.getPosition(),circle);
